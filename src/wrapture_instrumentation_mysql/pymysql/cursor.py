...
cursor descends from, recorded as database events.

PyMySQL's cursors are pure Python, so each method is bound in place
on `pymysql.cursors.Cursor`. `execute`, `executemany` and `callproc`
are the doors every query passes through, whichever cursor class the
application chose: `DictCursor`, `SSCursor` and `SSDictCursor` all
inherit the three unchanged (the unbuffered cursors override only
the internals beneath them), so one binding each on `Cursor` covers
every cursor class, and the events say `Cursor.execute` whichever
subclass ran.

Each binding is a leaf, which is what keeps one application call to
one event: `executemany` runs its batches through `execute` (once
per multi-row INSERT it assembles, or once per parameter set for any
...
above the driver's client-side interpolation (`mogrify`), so the SQL
text the event can carry is the template with its placeholders, and
