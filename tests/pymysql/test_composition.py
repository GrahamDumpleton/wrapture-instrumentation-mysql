...
alongside, over the pymysql dialect: the default leaf keeps the
...
# then sees them (a named parameter compiles to PyMySQL's spelling).
...
def driver_events(tape: Tape) -> list[Event]:
    """The PyMySQL events that belong to the workload, leaving out the
...
def test_the_default_leaf_keeps_the_driver_out(mysql: Server) -> None:
    ...
    assert at(tape, "pymysql.connections:Connection.connect") == []
    assert at(tape, "pymysql.connections:Connection.commit") == []
...
def test_leaf_off_nests_the_driver_beneath_each_statement(mysql: Server) -> None:
    ...
    for event in (create, select):
        assert event.path == "pymysql.cursors:Cursor.execute"
        parent = tape.parent_of(event)
        assert parent is not None and parent.path == EXECUTE

    assert insert.path == "pymysql.cursors:Cursor.executemany"
...
def test_raw_driver_use_beside_the_engine_records_at_top_level(
    ...
    assert raw.path == "pymysql.cursors:Cursor.execute"
