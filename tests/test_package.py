...
import wrapture_instrumentation_mysql as package
...
def test_importing_the_package_imports_nothing_else() -> None:
    ...
    appeared = run_snippet(
        ...
        "import wrapture_instrumentation_mysql\n"
        "print(sorted(set(sys.modules) - before))\n"
    )

    assert appeared.strip() == "['wrapture_instrumentation_mysql']"
...
def test_each_entry_point_loads_an_instrumentation_named_for_its_target(
    ...
    # `name = "pymysql"` and the class's `target` agree.
...
def test_the_readme_table_lists_every_target() -> None:
    ...
    for point in ENTRY_POINTS:
        ...
        link = (
            "https://github.com/GrahamDumpleton/wrapture-instrumentation-mysql"
