"""Drive PyMySQL against a MySQL server with the instrumentation
applied.
...
DictCursor, a stored procedure, a transaction with a rollback and a
failing statement; then the same with SQL text recording on. Each
...
def add_otel_sink() -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            " demo through `just demo-pymysql --otel`, which overlays"
            " wrapture[otel] for the run"
        ) from error

    wrapture.add_sink(wrapture.otel.sink(service_name="wrapture-pymysql-demo"))


def connect_kwargs(url: str) -> dict[str, Any]:
    """The keyword arguments PyMySQL takes, from the URL form the
...
def queries(url: str) -> None:
    ...
    with pymysql.connect(**connect_kwargs(url)) as connection:
...
def main(arguments: list[str] | None = None) -> None:
    """Run the demo: apply the instrumentation, drive PyMySQL against
    the server, print the live stream and the tree, and flush any
    exporters."""

    parser = argparse.ArgumentParser(
        prog="demo.pymysql",
        description="Drive PyMySQL against a MySQL server with the"
    ...
    with wrapture.timeline() as tape:
        ...
        with wrapture.instrumentation("pymysql", statement=True):
            queries(url)
    ...
    if options.otel:
        ...
        print()
        print("== otel ==")
        print(f"spans flushed to {endpoint} as service wrapture-pymysql-demo")
